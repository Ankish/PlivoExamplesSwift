...
class CallViewController: UIViewController {
     ...
     */
    // MARK: - class method
    class func storyBoardControllerForOutGoing(callerId: String,callerName : String,isOutGoing : Bool) -> CallViewController {
        ...
        vc.callerName = callerName
        vc.isOutGoing = isOutGoing
        
        if (isOutGoing) {
            let info = CallInfo(displayName: callerName,number: callerId, createdDate: Date(), isIncoming: false)
    ...
    private func setUp() {
        ...
        if isOutGoing {
            ...
            CallKitInstance.sharedInstance.reportOutGoingCall(with: UUID(), handle: self.callerId)
            self.callingInfoLabel.text = "Calling \(callerName).."
        } else {
            self.callingInfoLabel.text = "Call from \(callerName).."
        }
...
extension CallViewController : PlivoEndpointDelegate  {
    ...
    func onOutgoingCallRinging(_ call: PlivoOutgoing!) {
        DispatchQueue.main.async {
            ...
            self.callingInfoLabel.text = "Ringing \(self.callerName)..."
        }
    }
    ...
    func onOutgoingCallAnswered(_ call: PlivoOutgoing!) {
        DispatchQueue.main.async {
            ...
            self.callingInfoLabel.text = "Answered \(self.callerName)"
        }
    }
